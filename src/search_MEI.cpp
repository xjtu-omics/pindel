...
bool is_concordant(const bam1_t* read, unsigned int insert_size)
{
   ...
   return (unsigned int) cabs(read->core.isize) < read->core.l_qseq + 2 * insert_size;
}
...
static int fetch_disc_read_callback(const bam1_t* alignment, MEI_data* mei_data, UserDefinedSettings* userSettings)
{
   if (!(alignment->core.flag & BAM_FUNMAP || alignment->core.flag & BAM_FMUNMAP) && // Both ends are mapped.
         ...
         (alignment->core.tid != alignment->core.mtid ||
          (int)cabs(alignment->core.pos - alignment->core.mpos) > userSettings->MIN_DD_MAP_DISTANCE)) {
